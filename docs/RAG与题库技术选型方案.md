...
> 版本：v2.1 ｜ 日期：2026-03-29 ｜ 状态：**已代码级走通并真实验证（三路线 + 评测 harness 已落地，fake 离线 + real 百炼真实质量矩阵已产出，见 §4.4）**
...
| 裁决 | 内容 | 本文立场 |
| --- | --- | --- |
| ADR-0007 | 单文档出题不引独立向量数据库（Chroma 等） | **已撤销（superseded by ADR-0010，2026-03-29）**：跨文档检索触发其反转条件，生产 PG 改用 **pgvector 扩展**（VECTOR(1024)+HNSW），见 §2.3；单文档与 dev/SQLite 仍走内存余弦。**独立向量数据库服务仍不引入** |
| ADR-0008 | 默认不引入 LangChain/LangGraph | **已撤销（superseded by ADR-0009，2026-03-29）**：LangGraph 编排已引入并作为知识库出题生产默认（路线③），见 §5；**仍不引** LangChain 全套及其检索层封装（`PGVector`/`EnsembleRetriever`） |
...
- **real 真实模式**（已跑通，2026-03-29，供应商：阿里百炼 DashScope OpenAI 兼容）：
  ```bash
...
### 4.4 real 真实模式实验结果（2026-03-29，百炼 qwen-plus + text-embedding-v3）
