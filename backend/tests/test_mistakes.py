...
def test_wrong_answers_aggregated_by_knowledge_point(client, db_session):
    ...
    import_questions(db_session, items=_mk_items("学生观"))
    uid = _guest(client)

    _, out = _start_submit(client, uid, "学生观", 2, correct=False)
...
def test_repeat_wrong_increments_count(client, db_session):
    """同一考点反复错：错次累计（验收 2）。"""
    import_questions(db_session, items=_mk_items("教师观"))
    uid = _guest(client)

    _start_submit(client, uid, "教师观", 2, correct=False)
    _start_submit(client, uid, "教师观", 2, correct=False)
...
def test_correct_answers_do_not_enter_mistake_book(client, db_session):
    import_questions(db_session, items=_mk_items("教育观"))
    uid = _guest(client)

    _start_submit(client, uid, "教育观", 2, correct=True)
...
def test_mistake_repractice_starts_session(client, db_session):
    ...
    import_questions(db_session, items=_mk_items("法律法规"))
    uid = _guest(client)
    _start_submit(client, uid, "法律法规", 2, correct=False)

    r = client.post(
        "/api/sessions/start",
        json={"knowledge_point": "法律法规", "question_count": 2},
        headers={"X-Unionid": uid},
    )
    assert r.status_code == 200
    assert r.json()["knowledge_point"] == "法律法规"
