...
_ALLOWED_WITHOUT_BRANCH: dict[str, str] = {
    ...
    "对话历史": "多轮对话的历史块（teacher 的决策/作答/改写三处共用），只是排版",
    "格式纠正": "teacher 回答解析失败后**追加**在提示词末尾的格式纠正段（2026-03-05）："
    "分发仍靠「【答疑作答】」，它只要求把话说成规定的形状",
...
def test_回归_judge那次失效_题目评分标记必须有分支():
    """**这条钉的就是 2026-03-04 的真实失效**：`eval/judge.py` 的 docstring 声称
