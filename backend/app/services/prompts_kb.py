...
def kb_question_prompt(
    ...
    idx += 1
    # 引用硬校验（services/citation.py）：source_quote 会被程序逐字比对，
    # 因此提示词必须把"原样摘录"讲清楚 —— 模型默认倾向改写/概括，
    # 而改写过的引用在子串判定下等于编造（会整题被拦）。
    lines.append(
        f"{idx}. 每道题额外输出字段 source_quote：从你所依据的切片中**原样摘录**一句关键原文"
        "（10~40 字，必须是切片里真实存在的连续文字）。禁止改写、概括、合并句子，"
        "禁止添加《》书名号或出处前缀。该字段会被程序逐字比对，改写会导致整题被丢弃。"
    )
    idx += 1
    if scope:
